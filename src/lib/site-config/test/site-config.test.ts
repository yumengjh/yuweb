...
describe("site-config", () => {
  ...
  it("exposes shared site behavior config and message-key based structures", () => {
    ...
    expect(zhT(siteConfig.identity.name)).toBe("鱼梦江湖");
  ...
  it("keeps visible copy in messages while base config only stores keys", () => {
    ...
    expect(enT(siteConfig.footer.summary)).toContain("continuously evolving personal space");
  });
  ...
  it("filters domain-restricted footer meta items by hostname with shared rules", () => {
    const localhostVisible = (siteConfig.footer.metaItems as readonly SiteFooterMetaItemConfig[]).filter((item) =>
      matchesDomainRule("localhost", item.domains),
    );
    const externalVisible = (siteConfig.footer.metaItems as readonly SiteFooterMetaItemConfig[]).filter((item) =>
      matchesDomainRule("other-domain.com", item.domains),
    );
